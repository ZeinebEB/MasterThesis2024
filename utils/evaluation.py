...
def run_evaluation(net,loader,encoder,check_perf=False, detection_loss=None,segmentation_loss=None,losses_params=None):
        ...
        if(detection_loss!=None):
            ...
            if(check_perf):
                ...
                for pred_obj,true_obj in zip(out_obj,labels):
                    ...
                    metrics.updateDET(np.asarray(encoder.decode(pred_obj,0.05)),true_obj,
                                threshold=0.2,range_min=5,range_max=100) 
        ...
        if(segmentation_loss!=None):
            ...
            if(check_perf):
                ...
                for pred_map,true_map in zip(out_seg,label_freespace):
                    ...
                    metrics.updateSEG(pred_map[0],true_map) 
    ...
    mAP,mAR, mIoU = metrics.GetMetrics(detection_loss,segmentation_loss)

    return {'loss':running_loss, 'mAP':mAP, 'mAR':mAR, 'mIoU':mIoU}

#decision=0 --> detection=1
#decision=1 --> segmentation=1



def run_FullEvaluation(net,loader,encoder,iou_threshold=0.5,decision=1):
...
#         print('  mIoU',mIoU*100,'%')
        ...
        print('Generating Predictions...')
        predictions = {'prediction':{'freespace':[]},'label':{'freespace':[]}}
        for i, data in enumerate(loader):
