...
from FFTRadNet.model.Deeplab.DeepLabEncoder import DeepLabEncoder
...
class DeepLabV3Encoder(nn.Module):
    def __init__(self, n_channels=32, n_classes=1, detection_head=False, segmentation_head=True):
        ...
        # Initialize DeepLab encoder
        self.encoder = DeepLabEncoder()

        # Define the decoder for processing outputs from DeepLab
        self.RA_decoder = RangeAngle_Decoder()

        # Detection and segmentation heads
        self.detection = DetectionHead(input_angle_size=224, reg_layer=2)
    ...
    def forward(self, x):
        ...
        # Ensure the encoder features are suitable for the RA decoder
        ra_features = {
            'x4': encoder_features['out'],
            'x3': encoder_features['aux'],
            'x2': encoder_features['low_level']
        }

        # Pass through the RA decoder
        RA = self.RA_decoder(ra_features)
