...
class RangeAngle_Decoder(nn.Module):
    ...
    def forward(self, features):
        ...
        # print("Shape of self.deconv4(T4):", self.deconv4(T4).shape)
        # print("Shape of T3:", T3.shape)
        T4_deconv = self.deconv4(T4)  # This has shape: torch.Size([2, 16, 128, 512])

        # Resize T3 to match the spatial dimensions of T4_deconv
        T3_resized = F.interpolate(T3, size=(T4_deconv.size(2), T4_deconv.size(3)), mode='bilinear',
                                   align_corners=False)

        # Now, you can concatenate them along the channel dimension
        ...
        # Resize S4_deconv to match the desired width of 512
        S4_deconv_resized = F.interpolate(S4_deconv, size=(256, 224), mode='bilinear', align_corners=False)

        # print("Shape of self.S4_deconv_resized(T4):", S4_deconv_resized.shape)
        # print("Shape of T2:", T2.shape)

        # Now, S4_deconv_resized can be concatenated with T2
...
class Unet_fft(nn.Module):
    def __init__(self,detection_head=True, segmentation_head=True):
