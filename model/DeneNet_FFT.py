...
from FFTRadNet.model.DenseNet import DenseNetEncoder
...
class Dense_fft(nn.Module):
    def __init__(self,depth=121, growth_rate=32, reduction=0.5, bottleneck=True, dropRate=0.0,detection_head=True,segmentation_head=True):

        super(Dense_fft, self).__init__()
        # Initialize the U-Net encoder
        self.detection_head = detection_head
        self.segmentation_head = segmentation_head

        self.densenet_encoder = DenseNetEncoder(depth, growth_rate, reduction, bottleneck, dropRate)
    ...
    def forward(self, x):
        ...
        features= self.densenet_encoder(x)
