...
class RangeAngle_Decoder(nn.Module):
    def __init__(self, ):
        ...
        # Top-down layers
        self.deconv4 = nn.ConvTranspose2d(8, 16, kernel_size=3, stride=(2, 1), padding=1, output_padding=(1, 0))
        ...
        self.conv_block3 = BasicBlock(192, 192)

        self.L3 = nn.Conv2d(24, 224, kernel_size=1, stride=1, padding=0)
        self.L2 = nn.Conv2d(24, 224, kernel_size=1, stride=1, padding=0)
...
class EfficientNetEnc(nn.Module):
    def __init__(self, n_channels=32, n_classes=1, detection_head=False, segmentation_head=True):
        super(EfficientNetEnc, self).__init__()
        self.detection_head_flag = detection_head
        self.segmentation_head_flag = segmentation_head

        # Custom MIMO pre-encoder, adjusting input dimensions
        self.mimo_pre_encoder = MIMO_PreEncoder(in_layer=n_channels, out_layer=3)

        # Initialize EfficientNet encoder
        self.encoder = PretrainedEfficientNet.from_pretrained('efficientnet-b2')

        # Define the decoder for processing outputs from EfficientNet
        self.RA_decoder = RangeAngle_Decoder()

        # Detection and segmentation heads as defined before
        self.detection_head_module = DetectionHead(input_angle_size=224, reg_layer=2)  # Adjust input angle size as needed
    ...
    def extract_features(self, x):
        ...
        for idx, block in enumerate(self.encoder._blocks):
            x = block(x)
            if idx in {2, 4, 10}:
                features[f'x{len(features) + 2}'] = x

        features['x4'] = self.encoder._conv_head(x)
        features['x4'] = self.encoder._bn1(features['x4'])
        features['x4'] = self.encoder._swish(features['x4'])
    ...
    def forward(self, x):
        ...
        if self.detection_head_flag:
            d = self.detection_head_module(RA)
            Y = F.interpolate(d, (128, 224))
            out['Detection'] = Y

        if self.segmentation_head_flag:
            s = self.freespace(RA)
